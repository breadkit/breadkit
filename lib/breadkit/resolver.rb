  ...
  class Resolver
    ...
    def resolve_pins(item, part)
      ...
      part.pins.each_with_index do |definition, index|
        ...
        anchor = explicit_pin(requested, definition, index, part)
        anchor ||= explicit_pin(item[:attrs], definition, index, part)
    ...
    def explicit_pin(source, definition, index, part)
    ...
    def footprint_pin(item, definition, part)
      ...
      row = %w[a b c d e f g h i j][row_index + offset[1].to_i]
      "#{row}#{at.col + offset[0].to_i}"
    ...
    def resolve_wires(components)
      wires = []
      occupied = components.values.flat_map { |component| component.pins.values.map(&:hole_id) }.compact.each_with_object({}) { |id, set| set[id] = true }
      @document.supplies.each { |item| [item[:plus], item[:minus]].each { |id| occupied[id] = true if @board.hole(id) } }
      reserved = {}
      ...
      @document.wires.each_with_index do |item, index|
        ...
        endpoints.each_with_index do |endpoint, side|
          if parsed[side]&.kind == :pin
            ...
            candidates = @board.strip(pin_hole)&.hole_ids
